...
class FunctionRepositoryTest {
    ...
    void setUp() {
        ...
        clearFunctionsTable();
        clearUsersTable();
        // Используем уникальные имена пользователей для избежания конфликтов
        String uniqueSuffix = String.valueOf(System.currentTimeMillis()) + "_" + Thread.currentThread().getId();
        try {
            User user = new User("bob_" + uniqueSuffix, "123");
            userId1 = userRepository.insert(user);
            User user2 = new User("pop_" + uniqueSuffix, "456");
            userId2 = userRepository.insert(user2);
        } catch (RuntimeException e) {
            // Если вставка не удалась, пробуем еще раз с другим суффиксом
            uniqueSuffix = String.valueOf(System.nanoTime()) + "_" + Thread.currentThread().getId();
            User user = new User("bob_" + uniqueSuffix, "123");
            userId1 = userRepository.insert(user);
            User user2 = new User("pop_" + uniqueSuffix, "456");
            userId2 = userRepository.insert(user2);
    ...
    private void clearFunctionsTable() {
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Сначала удаляем все точки (из-за внешних ключей)
            try {
                stmt.executeUpdate("DELETE FROM points");
            } catch (Exception e) {
                // Игнорируем ошибки, если таблица не существует или пуста
            }
            // Затем удаляем все функции
            try {
    ...
    private void clearUsersTable() {
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Затем удаляем все функции (из-за внешних ключей)
            try {
                stmt.executeUpdate("DELETE FROM functions");
            } catch (Exception e) {
                // Игнорируем ошибки, если таблица не существует или пуста
