...
public class PointServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        // /api/points - получить все точки (доступ - ADMIN)
        if (pathInfo == null || "/".equals(pathInfo)) {
            ...
            logger.info("All points sent to ADMIN");
            return;
        }
        // убираем начальный '/'
        ...
        // /api/points/function/{functionId}/range - получить точки в диапазоне с сортировкой
        if (pathInfo.startsWith("function/") && pathInfo.contains("/range")) {
            String rest = pathInfo.substring("function/".length());
            String functionIdStr = rest.substring(0, rest.indexOf("/range"));
            try {
                ...
                points.sort((p1, p2) -> {
                    ...
                    switch (finalSortBy) {
                        case "xValue":
                            result = Double.compare(p1.getXValue() != null ? p1.getXValue() : 0.0,
                                                   p2.getXValue() != null ? p2.getXValue() : 0.0);
                            break;
                        case "yValue":
                            result = Double.compare(p1.getYValue() != null ? p1.getYValue() : 0.0,
                                                   p2.getYValue() != null ? p2.getYValue() : 0.0);
                            break;
                        default:
                            result = Double.compare(p1.getXValue() != null ? p1.getXValue() : 0.0,
                                                   p2.getXValue() != null ? p2.getXValue() : 0.0);
                ...
                resp.setContentType("application/json");
                resp.getWriter().print(objectMapper.writeValueAsString(points));
                logger.info("Отправлены точки функции {} в диапазоне", functionId);
                return;
            } catch (NumberFormatException e) {
                sendError(resp, HttpServletResponse.SC_BAD_REQUEST, "Invalid functionId or range parameters"); // 400
                return;
            }
        }
        ...
        if (pathInfo.startsWith("function/") && !pathInfo.contains("/x/") && !pathInfo.contains("/range")) {
