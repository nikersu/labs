...
public class PointRepository {
    ...
    // получение всех точек (для ADMIN)
    public List<Point> findAll() {
        logger.info("Start of operation: obtaining all points");
        List<Point> points = new ArrayList<>();
        try (Connection conn = DatabaseConnection.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT function_id, x_value, y_value FROM points")) {
            while (rs.next()) {
                points.add(Mapper.mapToPoint(rs));
            }
        } catch (SQLException e) {
            logger.error("Error when getting all points: {}", e.getMessage());
            return new ArrayList<>();
        }
        logger.info("The operation is complete: {} points have been found", points.size());
        return points;
    }
