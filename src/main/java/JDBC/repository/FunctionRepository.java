...
public class FunctionRepository {
    ...
    public Integer insert(Function function) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Устанавливаем JSON поля (могут быть null)
            if (function.getXValuesJson() != null) {
                stmt.setString(4, function.getXValuesJson());
            } else {
                stmt.setNull(4, Types.VARCHAR);
            }
            if (function.getYValuesJson() != null) {
                stmt.setString(5, function.getYValuesJson());
            } else {
                stmt.setNull(5, Types.VARCHAR);
            }
            if (function.getCount() != null) {
                stmt.setInt(6, function.getCount());
            } else {
            ...
            try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    ...
                    logger.info("Operation completed: function added with ID {} (saved as whole, not point by point)", id);
                    return id;
                } else {
    ...
    public boolean update(Function function) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setInt(7, function.getId());
            int affectedRows = stmt.executeUpdate();
            boolean success = affectedRows > 0;
            logger.info("Operation completed: the {} function has been updated (saved as whole, not point by point)", success ? "" : "not ");
            return success;
        } catch (SQLException e) {
